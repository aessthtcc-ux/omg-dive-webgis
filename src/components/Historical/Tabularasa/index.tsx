...
interface WildlifeItem {
  id: number; name: string; scientificName: string; image: string;
  likelihood: 1 | 2 | 3 | 4 | 5; likelihoodLabel: string;
  bestSeason: string; depth: string; behavior: string; tags: string[];
  scubagoSightings?: number; scubagoSource?: boolean;
}
...
const wildlifeRawData = [
  { id: 1, name: "Green Sea Turtle", scientificName: "Chelonia mydas",
    ...
    behaviorKey: "Often resting on the wreck structure or grazing on algae growth along the hull.",
  ...
  { id: 2, name: "Hawksbill Turtle", scientificName: "Eretmochelys imbricata",
    ...
    behaviorKey: "Feeds on sponges attached to the wreck. More active during daytime dives.",
  ...
  { id: 3, name: "Moray Eel", scientificName: "Gymnothorax spp.",
    ...
    likelihood: 5 as const, likelihoodLabelKey: "Very Common", bestSeason: "Year-round", depth: "15 – 33 m",
    behaviorKey: "Inhabits crevices and engine room openings. Frequently spotted during wreck penetration.",
    tags: ["Resident", "Nocturnal"] },
  { id: 4, name: "Angelfish", scientificName: "Pomacanthidae spp.",
    image: "https://cdn.divessi.com/cached/Wildlife_Angelfish_Shutterstock-Rich-Carey.jpg/400.jpg",
    likelihood: 5 as const, likelihoodLabelKey: "Very Common", bestSeason: "Year-round", depth: "5 – 25 m",
    behaviorKey: "Schools around the coral-encrusted upper deck. Bold and approachable for photography.",
  ...
  { id: 5, name: "Butterfly Fish", scientificName: "Chaetodontidae spp.",
    ...
    behaviorKey: "Pairs patrol coral growth on the wreck hull, feeding on coral polyps and small invertebrates.",
    tags: ["Reef Fish", "Photogenic"], scubagoSource: true },
  { id: 6, name: "Clownfish", scientificName: "Amphiprioninae spp.",
    ...
    behaviorKey: "Found sheltering in anemones growing on the wreck deck and railings. Active year-round.",
    tags: ["Anemone Host", "Resident"], scubagoSource: true },
  { id: 7, name: "Stingray", scientificName: "Dasyatidae spp.",
    image: "https://cdn.divessi.com/cached/Wildlife_Stingray_iStock-Extreme-Photographer.jpg/300.jpg",
    likelihood: 2 as const, likelihoodLabelKey: "Uncommon", bestSeason: "Aug – Nov", depth: "20 – 33 m",
    behaviorKey: "Rests camouflaged on sandy seabed near the wreck base. Best spotted during early morning dives.",
    tags: ["Benthic", "Sandy Bottom"], scubagoSource: true },
  { id: 8, name: "Sweetlips & Grunts", scientificName: "Haemulidae spp.",
    image: "https://cdn.divessi.com/cached/Wildlife_Sweetlips_Grunts_Shutterstock-aquapix.jpg/300.jpg",
    likelihood: 3 as const, likelihoodLabelKey: "Occasional", bestSeason: "Jun – Oct", depth: "10 – 30 m",
    behaviorKey: "Aggregates in small schools around the wreck superstructure during daytime rest periods.",
    tags: ["Schooling", "Wreck Dweller"], scubagoSource: true },
  { id: 9, name: "Parrotfish", scientificName: "Scaridae spp.",
    ...
    behaviorKey: "Grazes on algae and coral encrusting the hull. Produces white sand as a byproduct of feeding.",
    tags: ["Grazer", "Reef Fish"], scubagoSource: true },
  { id: 10, name: "Pufferfish", scientificName: "Tetraodontidae spp.",
    image: "https://cdn.divessi.com/cached/Wildlife_Puffer_Fish_AdobeStock-JirinaVFR.jpg/300.jpg",
    likelihood: 2 as const, likelihoodLabelKey: "Uncommon", bestSeason: "Feb – Mar, Sep", depth: "5 – 30 m",
    behaviorKey: "Solitary and slow-moving, often hovering near crevices. Inflates when threatened.",
    tags: ["Solitary", "Venomous"], scubagoSource: true },
];
    ...
    <div className="flex gap-1 items-center">
      {Array.from({ length: 5 }).map((_, i) => (
        <motion.div key={i} initial={{ scaleY: 0 }} animate={{ scaleY: 1 }}
          transition={{ delay: i * 0.06, duration: 0.3, ease: "easeOut" }} style={{ originY: 1 }}
  ...
  return createPortal(
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ opacity: 0, scale: 0.92, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.92, y: 16 }} transition={{ duration: 0.25, ease: "easeOut" }}
        ...
        <div className="overflow-y-auto" style={{ maxHeight: "calc(100dvh - 2rem)" }}>
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {animal.tags.map((tag) => (
                <span key={tag} className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-lg ${cfg.bg} ${cfg.color} border ${cfg.border}`}>{tag}</span>
              ))}
            </div>
          </div>
        </div>
...
const WildlifeSightings = () => {
  ...
  const wildlifeData: WildlifeItem[] = wildlifeRawData.map(raw => ({
    id: raw.id, name: raw.name, scientificName: raw.scientificName, image: raw.image,
    likelihood: raw.likelihood, likelihoodLabel: raw.likelihoodLabelKey,
    bestSeason: raw.bestSeason, depth: raw.depth, behavior: raw.behaviorKey,
    tags: raw.tags, scubagoSightings: (raw as any).scubagoSightings, scubagoSource: (raw as any).scubagoSource,
  }));
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="relative">
        <button onClick={() => scrollByCard("left")} disabled={!canScrollLeft} aria-label="Previous"
          className={`absolute left-0 -translate-x-1/2 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-white dark:bg-white/10 shadow-lg border border-gray-100 dark:border-white/10 flex items-center justify-center transition-all duration-200 ${canScrollLeft ? "opacity-100 hover:bg-primary hover:border-primary hover:text-white cursor-pointer" : "opacity-0 pointer-events-none"}`}>
          <ChevronLeft size={18} className="text-dark dark:text-white" />
        </button>
        <button onClick={() => scrollByCard("right")} disabled={!canScrollRight} aria-label="Next"
          className={`absolute right-0 translate-x-1/2 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-white dark:bg-white/10 shadow-lg border border-gray-100 dark:border-white/10 flex items-center justify-center transition-all duration-200 ${canScrollRight ? "opacity-100 hover:bg-primary hover:border-primary hover:text-white cursor-pointer" : "opacity-0 pointer-events-none"}`}>
          <ChevronRight size={18} className="text-dark dark:text-white" />
        </button>
        <div ref={scrollRef} className="flex overflow-x-auto gap-4 snap-x snap-mandatory px-1 pb-2"
          style={{ scrollbarWidth: "none", msOverflowStyle: "none" } as React.CSSProperties}>
          {wildlifeData.map((animal, idx) => {
            const cfg = likelihoodConfig[animal.likelihood];
            return (
              <button key={animal.id} ref={(el) => { cardRefs.current[idx] = el; }}
                onClick={() => setSelectedAnimal(animal)} aria-label={`View ${animal.name}`}
                ...
                <div className="relative rounded-2xl overflow-hidden shadow-md border-2 border-transparent group-hover:border-primary group-hover:-translate-y-1 group-hover:shadow-lg transition-all duration-300 aspect-[3/4]">
                  <img src={animal.image} alt={animal.name} className="w-full h-full object-cover grayscale-[30%] group-hover:grayscale-0 group-hover:scale-105 transition-all duration-500" />
        ...
        <div className="flex justify-center gap-1.5 mt-4">
          {wildlifeData.map((_, i) => (
            <button key={i} onClick={() => cardRefs.current[i]?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "start" })}
              aria-label={`Go to ${i + 1}`}
              className={`h-1.5 rounded-full transition-all duration-300 ${activeIndex === i ? "w-6 bg-primary" : "w-1.5 bg-gray-300 dark:bg-white/20"}`} />
          ))}
        </div>
      </div>
      <div className="flex items-start gap-1.5 pt-1">
        <Info size={11} className="text-gray-400 mt-0.5 flex-shrink-0" />
        <p className="text-[10px] text-gray-400">{t("Likelihood from species ecology & regional surveys.")}</p>
      </div>
      <AnimatePresence>
        {selectedAnimal && <DetailModal animal={selectedAnimal} onClose={() => setSelectedAnimal(null)} />}
      </AnimatePresence>
    </div>
  );
};
