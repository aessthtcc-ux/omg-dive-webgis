          ...
          <div className="inline-flex p-1 md:p-1.5 bg-gray-100 dark:bg-white/5 rounded-xl md:rounded-2xl border border-gray-200 dark:border-white/10 shadow-xl w-full md:w-auto">
            {["tabularasa", "poso"].map((tab) => (
              <button key={tab} onClick={() => setActiveWreck(tab)}
                ...
                {tab === "tabularasa" ? t("Tabularasa Wreck") : t("Poso Wreck")}
              </button>
            ))}
          </div>
        </div>
...
const LightboxPortal = ({ images, title, currentSlide, setCurrentSlide, onClose }: {
  images: string[]; title: string; currentSlide: number;
  ...
  return createPortal(
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 z-[9999] bg-black/90 flex items-center justify-center p-4 sm:p-8" onClick={onClose}>
      <motion.div initial={{ scale: 0.94, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.94, opacity: 0 }}
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/10 shrink-0">
          ...
          <button onClick={onClose} className="p-1.5 bg-white/10 hover:bg-red-500 text-white rounded-lg transition-colors border border-white/10"><X size={15} /></button>
        </div>
        <div className="relative flex-1 min-h-0 flex items-center justify-center bg-[#0a0a0a] overflow-hidden">
          <AnimatePresence mode="wait">
            <motion.img key={currentSlide} src={images[currentSlide]}
              ...
              style={{ maxHeight: "calc(100dvh - 12rem)" }} alt={`${title} ${currentSlide + 1}`} />
          </AnimatePresence>
          <button onClick={(e) => { e.stopPropagation(); setCurrentSlide(p => (p - 1 + images.length) % images.length); }}
            className="absolute left-3 p-2 bg-black/60 hover:bg-primary text-white rounded-full border border-white/20 transition-colors"><ChevronLeft size={16} /></button>
          <button onClick={(e) => { e.stopPropagation(); setCurrentSlide(p => (p + 1) % images.length); }}
            className="absolute right-3 p-2 bg-black/60 hover:bg-primary text-white rounded-full border border-white/20 transition-colors"><ChevronRight size={16} /></button>
        </div>
...
const TabularasaContent = () => {
  ...
  const [marineWeather, setMarineWeather] = useState<MarineData>({
    ...
    waterTemp: 0, windSpeed: 0, precipitation: 0, loading: true, error: null,
  });
    ...
    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.7 }}
          ...
          <div className="absolute top-5 md:top-10 left-5 md:left-10 z-20">
            <div className="flex items-center gap-2 bg-white/25 w-fit px-3 md:px-4 py-1 rounded-full border border-white/10">
              <Anchor size={13} />
              <span className="text-[10px] font-bold uppercase tracking-widest">{t("National Maritime Legacy")}</span>
            </div>
          </div>
          <div className="relative z-10 mt-8 md:mt-0">
            <h2 className="text-3xl md:text-5xl lg:text-6xl font-black mb-3 md:mb-6 tracking-tight leading-none">{t("Tabularasa Wreck")}</h2>
            <p className="text-sm md:text-lg text-blue-50/80 max-w-md leading-relaxed font-medium">
              {t("An STP training ship that found its final resting place in 1995. Now a thriving artificial reef and a symbol of Indonesia's maritime development.")}
            </p>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.7, delay: 0.15 }}
          ...
          <div className="col-span-2 row-span-1 relative rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden cursor-pointer group"
            ...
            <img src={tabularasaImages[0]} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt={t("Tabularasa Main")} loading="lazy" />
          ...
          <div className="relative rounded-[1.5rem] md:rounded-[2rem] overflow-hidden cursor-pointer" onClick={() => { setCurrentSlide(2); setIsGalleryOpen(true); }}>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.5 }}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg md:text-xl font-black flex items-center gap-2 dark:text-white">
            <Waves className="text-primary" size={18} /> {t("Current Conditions")}
          </h3>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.6 }}
        ...
        <h3 className="text-xl md:text-2xl font-black mb-4 flex items-center gap-3 text-dark dark:text-white">
          <History className="text-primary" /> {t("Historical Background")}
        </h3>
        <div className="space-y-3">
          <p className="text-sm md:text-base text-gray-600 dark:text-gray-400 leading-relaxed text-justify">
            {t("The Tabularasa wreck was an STP training ship that sank in the waters of Pramuka Island in 1995. Damage in the engine room was identified as the primary cause of the sinking.")}
          </p>
          <p className="text-sm md:text-base text-gray-600 dark:text-gray-400 leading-relaxed text-justify">
            {t("Although the wreck is under 50 years old, Tabularasa holds national maritime historical value — particularly related to Indonesia's post-independence maritime education — giving the site sufficient significance to be developed as a marine ecotourism attraction.")}
          </p>
        </div>
      </motion.div>
        ...
        <div className="bg-gradient-to-br from-amber-500 to-orange-700 p-5 md:p-8 lg:p-10 relative overflow-hidden">
          ...
          <div className="relative z-10 flex items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="text-white/70 font-medium text-xs md:text-sm">{t("Species recorded at Tabularasa Wreck Site")}</p>
            </div>
      ...
      <AnimatePresence>
        {isGalleryOpen && (
          <LightboxPortal images={tabularasaImages} title={t("Tabularasa Wreck Documentation")}
            currentSlide={currentSlide} setCurrentSlide={setCurrentSlide} onClose={() => setIsGalleryOpen(false)} />
        )}
      </AnimatePresence>
    </motion.div>
  );
    ...
    <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.7 }}
        ...
        <div className="relative z-10 flex flex-col gap-5">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-white/80">{t("Ecotourism Suitability Assessment")}</span>
            </div>
            <h3 className="text-2xl md:text-3xl lg:text-4xl font-black text-white tracking-tight mb-0.5">{t("Site Potential")}</h3>
            <p className="text-white/70 font-medium text-xs">{t("Based on criteria by I. Dillenia et al. (2021)")}</p>
          </div>
      ...
      <div className="p-5 md:p-8 lg:p-10">
        ...
        <div className="mt-5 p-4 bg-primary/5 border border-primary/20 rounded-[1.5rem] flex flex-col md:flex-row items-start gap-3">
          ...
          <div>
            <p className="text-xs md:text-sm font-bold text-primary">{t("Tabularasa rests at 20–33 meters depth, ideal for Advanced Open Water certified divers.")}</p>
            <p className="text-[10px] text-gray-400 mt-1 flex items-center gap-1.5">
              <BookOpen size={10} /> {t("I. Dillenia et al. (2021). Suitability assessment, Pramuka Island.")}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="relative z-10 flex flex-col gap-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h3 className="text-2xl md:text-3xl font-black text-white tracking-tight mb-0.5">{t(season.label)}</h3>
              <p className="text-white/70 text-xs md:text-sm font-medium">{season.period}</p>
            </div>
            <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
              <span className="px-3 py-1 rounded-full bg-white/20 text-white text-[10px] font-black uppercase border border-white/20">{t(season.badge)}</span>
          ...
          <div className="flex gap-2 flex-wrap">
            {seasonData.map((s) => (
              <button key={s.key} onClick={() => setActiveSeason(s.key)}
                className={`px-3 md:px-4 py-1.5 rounded-xl text-xs font-bold transition-colors flex items-center gap-1.5 ${activeSeason === s.key ? "bg-white text-gray-900 shadow-md" : "bg-white/15 text-white hover:bg-white/25 border border-white/20"}`}>
                {s.icon}
      ...
      <AnimatePresence mode="wait">
        <motion.div key={activeSeason} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.3 }}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-8">
            <div className="space-y-3">
              <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">{t("Environmental Conditions")}</p>
              {[
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex-1">
                <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">{t("Season Highlights")}</p>
                <div className="space-y-2">
              ...
              <div className="p-4 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10">
                <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">{t("Visibility Comparison")}</p>
                {seasonData.map((s) => (
