  ...
  useEffect(() => {
    ...
    layerIds.forEach((id) => {
      if (geoData[id]?.features) {
        try { const layer = L.geoJSON(geoData[id]); group.addLayer(layer); }
      ...
      try {
        ...
        const layer = new GeoRasterLayer({
          ...
          pixelValuesToColorFn: (values: any) => {
            if (values[0] === georaster.noDataValue || values[0] === undefined || isNaN(values[0]) || values[0] === 0) return null;
            if (values.length >= 3) return `rgb(${Math.round(values[0])}, ${Math.round(values[1])}, ${Math.round(values[2])})`;
            const val = values[0];
            if (maxVal === minVal) return 'rgb(150, 150, 150)';
            const pct = (val - minVal) / (maxVal - minVal);
            let r = 0, g = 0, b = 0;
            if (pct < 0.25) { r = 0; g = Math.round(4 * pct * 255); b = 255; }
            else if (pct < 0.5) { r = 0; g = 255; b = Math.round(255 - 4 * (pct - 0.25) * 255); }
            else if (pct < 0.75) { r = Math.round(4 * (pct - 0.5) * 255); g = 255; b = 0; }
            else { r = 255; g = Math.round(255 - 4 * (pct - 0.75) * 255); b = 0; }
            return `rgba(${r}, ${g}, ${b}, 0.9)`;
          }
        });
        if (isMounted) { layer.addTo(map); layerRef.current = layer; map.fitBounds(layer.getBounds(), { padding: [50, 50], maxZoom: 20 }); }
      } catch (error) { console.error("[DEBUG ERROR] Gagal memuat GeoTIFF:", error); }
    };
    loadRaster();
...
const PANEL_W_DESKTOP = 360; // px lebar panel di desktop
...
const Mapping2D = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      const results = await Promise.all(
        fetchableLayers.map(async (config) => {
          try {
            const res = await fetch(config.filePath);
            if (res.ok) return { id: config.id, data: await res.json() };
  ...
  const toggleGroup = (groupId: string) => {
    const next = !activeGroups[groupId];
    setActiveGroups(prev => ({ ...prev, [groupId]: next }));
    const group = layerGroups.find(g => g.groupId === groupId);
    if (group) setActiveSubLayers(prev => {
      const s = { ...prev };
      group.subLayers.forEach(sub => { s[sub.id] = next; });
      return s;
    });
  };
  ...
  // lebar panel saat slide: mobile = hampir full screen, desktop = 360px
  const isMobile = typeof window !== 'undefined' && window.innerWidth < 768;
  const panelSlideWidth = isMobile ? window.innerWidth - 60 : PANEL_W_DESKTOP;
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden bg-slate-950 pt-24">
          ...
          {layerGroups.flatMap(g => g.subLayers).map((config) => {
            ...
            return (
              <GeoJSON
                ...
                eventHandlers={{
                  contextmenu: (e) => {
                    const layer = e.target; const map = layer._map;
                ...
                onEachFeature={(feature, layer: any) => {
                  ...
                  if (config.isPolygon) {
                    ...
                    layer.bindPopup(`<div class="p-2 font-sans"><div class="text-[10px] font-black text-red-500 uppercase tracking-widest border-b border-gray-100 mb-2 pb-1">Area Boundary</div><div class="text-sm font-bold text-gray-800 uppercase">${config.title}</div><div class="text-[11px] text-gray-500 mt-1 italic font-mono">Center: ${center.lat.toFixed(5)}, ${center.lng.toFixed(5)}</div></div>`);
                  } else {
                    layer.bindPopup(`<div class="p-2 font-sans"><div class="text-[10px] font-black text-blue-500 uppercase tracking-widest border-b border-gray-100 mb-2 pb-1">Survey Segment</div><div class="text-sm font-bold text-gray-800 uppercase italic">${config.title}</div><div class="text-[11px] text-gray-500 mt-1 italic">Project: ${config.project}</div><div class="mt-3 text-[9px] bg-blue-50 text-blue-600 p-1 rounded text-center font-bold">${feature.properties?.description || 'Line segment recorded during survey'}</div></div>`);
                  }
                }}
              />
            );
        ...
        <AnimatePresence>
          {isBaseMapOpen && (
            <motion.div
              ...
              className="bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl border border-gray-200 dark:border-white/10 p-3 md:p-4 rounded-[1.5rem] md:rounded-[2rem] shadow-2xl w-40 md:w-48 flex flex-col gap-2 md:gap-3 mt-2 pointer-events-auto"
            >
              {baseMaps.map((map) => (
                <button
                  key={map.id}
      ...
      {/* ✅ FIX: TOMBOL TOGGLE TERPISAH dari motion.div — selalu kelihatan */}
      <div
        ...
        style={{
          left: isPanelOpen
            ? `calc(0.5rem + min(calc(100vw - 60px), ${PANEL_W_DESKTOP}px) + 0.5rem)`
            : '0.5rem',
        }}
      >
        <button
      ...
      {/* SIDE PANEL — slide tanpa tombol toggle di dalamnya */}
      <motion.div
      ...
      >
        <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl p-5 md:p-6 flex flex-col pointer-events-auto overflow-hidden h-full">
          ...
          <div className="flex-1 space-y-4 md:space-y-6 overflow-y-auto pr-1 md:pr-2 custom-scrollbar">
              ...
              <div key={group.groupId} className="space-y-2 md:space-y-3">
                <div className="flex items-center justify-between px-1 md:px-2">
                  <div className="flex items-center gap-2 md:gap-3">
                    ...
                    >
                      {activeGroups[group.groupId] ? <Eye size={15} /> : <EyeOff size={15} />}
                    </button>
                    <h3 className="text-[11px] md:text-[12px] font-black uppercase tracking-widest text-gray-800 dark:text-gray-200 flex items-center gap-2">
                      {group.icon} {group.title}
                    </h3>
                  </div>
                </div>

                <div className="pl-3 md:pl-4 border-l-2 border-gray-100 dark:border-white/5 ml-3 md:ml-4 space-y-2">
                  {group.subLayers.map((layer) => (
                    <div key={layer.id} className="group bg-gray-50/50 dark:bg-white/5 rounded-xl md:rounded-2xl border border-transparent transition-all overflow-hidden hover:border-gray-200 dark:hover:border-white/10">
                      <div className="p-2.5 md:p-3 flex items-center justify-between">
                        <div className="flex items-center gap-2 md:gap-3">
                          <button
                            onClick={() => toggleSubLayer(layer.id)}
                            className={`p-1.5 md:p-2 rounded-lg md:rounded-xl transition-all ${activeSubLayers[layer.id] ? 'bg-gray-800 dark:bg-gray-100 text-white dark:text-gray-900 shadow-md' : 'bg-gray-200 dark:bg-white/10 text-gray-400'}`}
                          >
                            {activeSubLayers[layer.id] ? <Eye size={13} /> : <EyeOff size={13} />}
                          </button>
                          <div>
                            <h4 className="text-[10px] font-bold text-gray-700 dark:text-gray-300 uppercase tracking-tight flex items-center gap-1.5">
                              {layer.title}
                              {(layer as any).isDummy && <span className="text-[8px] bg-purple-100 text-purple-600 px-1.5 rounded-sm">Soon</span>}
                            </h4>
                            <div className="flex items-center gap-1.5 mt-0.5">
                              <div className="w-5 h-0.5 rounded-full" style={{ backgroundColor: layer.color }} />
                      ...
                      <AnimatePresence>
                        {openDesc === layer.id && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="px-3 md:px-4 pb-3 md:pb-4 text-[9px] md:text-[10px] text-gray-500 italic border-t border-gray-100 dark:border-white/5 pt-2 md:pt-3 bg-white/50 dark:bg-black/20">
      ...
      {/* LEGEND DESKTOP — hidden di mobile */}
      <motion.div
